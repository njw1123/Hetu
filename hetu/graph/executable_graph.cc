...
static bool is_pipeline_stage_send_op(Operator& op) {
...
/*
static bool is_fused_pipeline_stage_send_op(Operator& op) {
  if (is_peer_to_peer_send_op(op)) {
    return true;
  }
  auto cur_op = op;
...
static Operator get_next_pipeline_stage_send_op(Operator& op) {
  if (is_peer_to_peer_send_op(op)) {
    return op;
  }
  auto cur_op = op;
...
static bool is_pipeline_stage_recv_op(Operator& op) {
  if (op->graph().GetSubGraph(op) == nullptr || op->graph().GetSubGraph(op)->subgraph_type() != SubGraphType::PIPELINE) {
    return false;
  }
...
/*
static bool is_fused_pipeline_stage_recv_op(Operator& op) {
  if (is_peer_to_peer_recv_op(op)) {
    return true;
  }
  auto cur_op = op;
...
static Operator get_last_pipeline_stage_recv_op(Operator& op) {
  if (is_peer_to_peer_recv_op(op)) {
    return op;
  }
  auto cur_op = op;
...
void ExecutableGraph::SortOptimizeComputeBridgeSubgraph() {
...
void ExecutableGraph::SortComputeOptimizeBridgeSubgraph() {
...
void ExecutableGraph::ComputeFunc(size_t& micro_batch_id, const OpRefList& topo, RuntimeContext& runtime_ctx,
  ...
  for (auto& op_ref : topo) {
    ...
    if (_run_level == RunLevel::COMPUTE_ONLY
        && (GetSubGraph(op) != nullptr && GetSubGraphOpType(op) == SubGraphOpType::UPDATE)) {
      continue;
    }
...
NDArrayList ExecutableGraph::Run(const Tensor& loss, const TensorList& fetches, 
  ...
  if (is_execute_plan_changed) {
    ...
    for (auto& op_ref : bw_topo) {
      ...
      accumulated_ops.insert(op_ref.get()->id()); // update op本身也是只运行一次的
      ...
      if (update_topo.size() >= 1 && update_topo.back().get()->num_outputs() >= 1) {
        auto& final_grad = update_topo.back().get()->output(0);
      ...
      if (is_optimizer_update_op(op)) {
        ...
        if (is_grad_reduce_op(grad_op) || is_sum_op(grad_op)) {
          ...
          if (is_grad_reduce_op(grad_op) && is_sum_op(grad_op->input(0)->producer())) {
            // HT_LOG_INFO << "share weight with dp, need to first sum " << grad_op->input(0)->producer()->inputs() << " and then reduce";
